...
export function PredictiveInsights({ progressItems, activity }: PredictiveInsightsProps) {
  ...
  const insights = useMemo(() => {
    ...
    const velocityPerDay = totalConsumed / activeDays;
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-semibold text-foreground">{t('advancedStats')}</h2>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              {velocityPerDay > 60
                ? `~${formatHours(velocityPerDay)} ${t('perDayAvg')}`
                : t('notEnoughData')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-4">
              <div className="text-center">
                ...
                <p className="text-xs text-muted-foreground mt-1">{t('daysInARow')}</p>
              </div>
                    ...
                    <div key={day} className="flex-1 flex flex-col items-center gap-0.5" title={`${day}: ${count}`}>
                      ...
                      <span className="text-[8px] text-muted-foreground">
                        {/^\d{4}-\d{2}-\d{2}$/.test(day)
                          ? new Date(day + 'T12:00:00').toLocaleDateString(lang === 'en' ? 'en-US' : 'ro-RO', { weekday: 'short' }).slice(0, 2)
                          : day.slice(0, 2)}
                      </span>
                    </div>
                  );
        ...
        <Card>
          <CardHeader className="pb-2">
            ...
            <CardDescription>{t('timeByGenre')}</CardDescription>
          </CardHeader>
              ...
              return (
                <div key={cat} className="space-y-0.5">
                  <div className="flex justify-between text-xs">
                    <span className="text-foreground truncate pr-2">{displayCategory(cat, lang)}</span>
        ...
        {predictions.length > 0 && (
          <Card className="sm:col-span-2">
            <CardHeader className="pb-2">
              ...
              <CardDescription>{t('basedOnPace')}</CardDescription>
            </CardHeader>
            <CardContent>
                      ...
                      <div className="flex justify-between items-baseline gap-2">
                        ...
                        {p.completionDate ? (
                          <p className="text-xs text-green-600 dark:text-green-400 shrink-0 font-medium">
                            ~{formatDate(p.completionDate, lang)}
                          </p>
                        ) : (
                          <p className="text-xs text-muted-foreground shrink-0">{t('unknownDate')}</p>
                        )}
                      </div>
                      <div className="flex items-center gap-2">
